...
from .hwp_extract import convert_hwp_to_blocks
from .hwp_extract import _check_libreoffice_available, _get_soffice_cmd
from .hwp_extract import extract_docx_blocks
import subprocess
...
class HwpAdapter:
    ...
    def to_blocks(self, hwp_path: str):
        """
        HWP → DOCX → 본문(text) / 표(table) 블록 반환 (RAG용)
        """
        hwp_path = Path(hwp_path).resolve()
        ...
        out_dir = hwp_path.parent / "hwptodocx"
        out_dir.mkdir(exist_ok=True)
        ...
        docx_path = out_dir / f"{hwp_path.stem}.docx"
        ...
        return {
            ...
            "text_blocks": blocks["text_blocks"],
            "table_blocks": blocks["table_blocks"],
        }
    ...
    def to_text(self, hwp_path: str, method: str | None = None) -> str:
        ...
        (테스트 / 유틸용)
        """
        return convert_hwp_to_blocks(hwp_path, method=method)
